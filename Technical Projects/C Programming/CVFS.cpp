////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Application program for the C project - Customised Virtual File System(CVFS)
//
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
typedef struct ufdt
{
    PFILETABLE ptrfiletable;    //pointer to file table
}UFDT;
...
void man(char *name)
{
    ...
    {
        printf("Description : Used to list name and inode number of all files \n");
        printf("Usage : ls-i\n");
    }
...
void DisplayHelp()
{
    printf("ls : To List names of all files\n");
    printf("ls-a : To List out all files\n");
    printf("ls-i : To List out names and inode numbers of all files\n");
...
void CreateDILB()    //It will create the linkedlist of the INODE
{
    int i = 1;
    PINODE newn = NULL;
    PINODE temp = head;
...
void InitialiseSuperBlock()   // this function is used to set the indice of UFDTARR and initialise its values with NULL
{
    int i = 0;
    while(i < MAXINODE)
    {
...
void Save_Files()
{
    ...
    while(i < MAXINODE)
    {
        int fd1 = creat(UFDTArr[i].ptrfiletable->ptrinode->FileName, 0777);
        write(fd1,UFDTArr[i].ptrfiletable->ptrinode->Buffer, 1024);
        i++;
    }
}
...
int main()
{
    ...
    InitialiseSuperBlock();  //auxilary data initiailization
    CreateDILB();            // hi pan auxilary data initialisation

    while(1)
    {
        fflush(stdin);     // hotel madhla table clean karn
        strcpy(str,"");   // str clean keli

        printf("\nMarvellous VFS :>");   // shell header

        fgets(str, 80, stdin);   //scanf("%[^'\n']s", str);  //fgets(kashyat, kiti, kuthun)

        count = sscanf(str, "%s %s %s %s", command[0], command[1], command[2], command[3]); //strtok pan vapru shakto, string la space pashi sodto
        ...
        if(count == 1)
        {
            ...
            else if(strcmp(command[0], "ls-a") == 0)
            {
                lsa_file();
            }
            else if(strcmp(command[0], "ls-i") == 0)
            {
                lsi_file();
            }
        ...
        else if(count == 3)
        {
            ...
            {
                ret = CreateFile(command[1], atoi(command[2]));   // CreateFile is an API in windows but here we used it as user defined function
                if(ret >= 0)
